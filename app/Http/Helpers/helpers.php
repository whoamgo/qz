...
function showRatings($avgRate) {
    ...
    if ($avgRate > 0) {
        ...
        for ($i = 1; $i <= $avgRating; $i++) {
            $ratings .= '<li class="star-list-item"><i class="las la-star"></i></li>';
        }
        if ($fraction > .25 && $fraction < .75) {
            $avgRating += 1;
            $ratings .= '<li class="star-list-item"><i class="las la-star-half-alt"></i></li>';
        }
    } else {
        $avgRating = 0;
    }
    ...
    for ($k = 1; $k <= $nonStar; $k++) {
        $ratings .= '<li class="star-list-item"><i class="lar la-star"></i></li>';
    }
    return $ratings;
}
function showTestimonialRatings($avgRate) {
    $ratings = '';
    if ($avgRate > 0) {
        $avgRating  = $avgRate;
        $integerVal = floor($avgRating);
        ...
        for ($i = 1; $i <= $avgRating; $i++) {
            $ratings .= '<span class="icon"><i class="fa-solid fa-star"></i></span>';
        }
        if ($fraction > .25 && $fraction < .75) {
            $avgRating += 1;
            $ratings .= '<span class="icon"><i class="fa-solid fa-star-half-stroke"></i></span>';
        }
    } else {
        $avgRating = 0;
    }
    $nonStar = 5 - intval($avgRating);
    for ($k = 1; $k <= $nonStar; $k++) {
        $ratings .= '<span class="icon"><i class="fa-regular fa-star"></i></span>';
    }
    return $ratings;
}
...
/**
 * Public website asset URL, cache-busted on the file's modification time so a
 * deploy invalidates the browser cache without a manual version bump.
