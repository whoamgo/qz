            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.frontend.sections.content', $key) }}" class="disableSubmission" method="POST" enctype="multipart/form-data">
                                    ...
                                    @foreach ($content as $imgKey => $image)
                                        <div class="col-md-4">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <x-image-uploader class="w-100" :imagePath="frontendImage($key, (isset($data->data_values->$imgKey) ? $data->data_values->$imgKey : ''),$section->element->images->$imgKey->size)" name="image_input[{{ $imgKey }}]" id="image-upload-input{{ $loop->index }}" :size="isset($section->element->images->$imgKey->size) ? $section->element->images->$imgKey->size : null" :required="false" />
                                ...
                                @elseif($content == 'icon')
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control iconPicker icon" autocomplete="off" name="{{ $k }}" value="{{ old($k, isset($data->data_values->$k) ? $data->data_values->$k : '') }}" required>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control select2" data-minimum-results-for-search="-1" name="{{ $selectName }}" required>
                                                    @foreach ($content->options as $selectItemKey => $selectOption)
                                                        <option value="{{ $selectItemKey }}" @if ((isset($data->data_values->$selectName) ? $data->data_values->$selectName : '') == $selectItemKey) selected @endif>{{ __($selectOption) }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    @elseif($k == 'slug')
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" name="slug" value="{{ old($k, isset($data->slug) ? $data->slug : '') }}" required>
                                            </div>
                                        </div>
                                    @else
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" name="{{ $k }}" value="{{ old($k, isset($data->data_values->$k) ? $data->data_values->$k : '' ) }}" required>
                                            </div>
                                        </div>
                                    @endif
                                @endif
                        ...
                        <div class="form-group">
                            <button type="submit" class="btn btn--primary @if(isset($section->element->slug) && !isset($data)) disabled @endif w-100 h-45">@lang('Submit')</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            @if(isset($section->element->slug) && $section->element->slug)
                ...
                $('[name=slug]').on('input',function(){
                    ...
                    if (slug) {
                        ...
                        `);
                        $.get("{{ route('admin.frontend.sections.element.slug.check',[$key, isset($data->id) ? $data->id : '']) }}", {slug:slug},function(response){
                            if (!response.exists) {
                                closestForm.find('.slug-verification').html(`
