...
<a href="{{ $url ?? locale_route('website.category.show', $category->slug) }}" class="w-card w-cat-card text-decoration-none">
    <span class="w-cat-icon" aria-hidden="true">
        @include('website.partials.category-icon', ['category' => $category])
    </span>
    <h3 class="w-card-title" style="font-size: var(--w-fs-base);">{{ $category->tr('name') }}</h3>
