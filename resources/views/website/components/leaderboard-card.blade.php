...
<div class="d-flex align-items-center gap-3 p-3 w-rank-row {{ $isMe ? 'is-me' : '' }}" style="border-bottom: 1px solid var(--w-border);">
    <span class="fw-bold text-center flex-shrink-0" style="width: 40px; font-size: var(--w-fs-lg);">
        @if ($rank === 1) <i class="bi bi-trophy-fill text-warning" role="img" aria-label="Rank 1"></i>
        @elseif ($rank === 2) <i class="bi bi-trophy-fill" style="color:#94a3b8;" role="img" aria-label="Rank 2"></i>
        @elseif ($rank === 3) <i class="bi bi-trophy-fill" style="color:#fb923c;" role="img" aria-label="Rank 3"></i>
        @else {{ $rank }} @endif
    </span>
