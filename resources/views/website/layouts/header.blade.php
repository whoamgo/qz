@php
    $navItems = [
        ...
        ['label' => 'Quizzes',         'route' => 'website.quizzes',               'icon' => 'bi-patch-question'],
        ['label' => 'Current Affairs', 'route' => 'website.current.affairs.index', 'icon' => 'bi-newspaper'],
        ['label' => 'Exams',           'route' => 'exams',                 'icon' => 'bi-mortarboard'],
        ['label' => 'GK',              'route' => 'website.category.show',         'icon' => 'bi-lightbulb', 'param' => 'general-knowledge'],
        ['label' => 'Leaderboard',     'route' => 'website.leaderboard',           'icon' => 'bi-trophy'],
        ['label' => 'Blog',            'route' => 'blog',                  'icon' => 'bi-journal-text'],
...
<header class="w-header">
    <div class="container">
        <div class="d-flex align-items-center gap-3 py-2">
            ...
            <a href="{{ route('home') }}" class="w-logo d-flex align-items-center gap-2 flex-shrink-0">
            ...
            {{-- Search --}}
            <div class="w-search-wrap ms-auto d-none d-lg-block">
                <form action="{{ route('website.search') }}" method="GET" role="search">
                    ...
                    <input type="search" name="q" class="form-control w-search-input"
                           placeholder="Search quizzes, topics, exams..."
                           value="{{ request('q') }}" autocomplete="off"
                           aria-label="Search quizzes"
                           data-suggest-url="{{ route('website.search.suggest') }}">
                </form>
                <div class="w-suggest" role="listbox" aria-label="Search suggestions"></div>
            </div>

            {{-- Account area --}}
            <div class="d-flex align-items-center gap-2 ms-auto ms-lg-0 flex-shrink-0">
                @guest
                    <a href="{{ route('user.login') }}" class="btn w-btn-outline btn-sm d-none d-sm-inline-flex">Login</a>
                    <a href="{{ route('user.register') }}" class="btn w-btn-primary btn-sm">Register</a>
                @else
                    <span class="w-xp-pill d-none d-md-inline-flex" title="Total XP">
                        <i class="bi bi-lightning-charge-fill" aria-hidden="true"></i>
                        {{ number_format($wXp->total_xp ?? 0) }} XP
                    </span>
                    <span class="w-level-pill d-none d-lg-inline-flex" title="Current level">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end shadow-sm" style="min-width: 232px;">
                            <li class="px-3 py-2 border-bottom">
                                ...
                                <small class="w-muted">&#64;{{ $wUser->username }}</small>
                            </li>
                            <li><a class="dropdown-item" href="{{ route('website.profile.index') }}"><i class="bi bi-person me-2"></i>Profile</a></li>
                            <li><a class="dropdown-item" href="{{ route('website.profile.quizzes') }}"><i class="bi bi-list-check me-2"></i>My Quizzes</a></li>
                            <li><a class="dropdown-item" href="{{ route('website.profile.progress') }}"><i class="bi bi-graph-up me-2"></i>My Progress</a></li>
                            <li><a class="dropdown-item" href="{{ route('website.profile.xp') }}"><i class="bi bi-lightning-charge me-2"></i>XP History</a></li>
                            <li><a class="dropdown-item" href="{{ route('website.profile.badges') }}"><i class="bi bi-award me-2"></i>Badges</a></li>
                            <li><a class="dropdown-item" href="{{ route('website.profile.streak') }}"><i class="bi bi-fire me-2"></i>Streak</a></li>
                            <li><a class="dropdown-item" href="{{ route('website.profile.bookmarks') }}"><i class="bi bi-bookmark me-2"></i>Bookmarks</a></li>
                            <li><a class="dropdown-item" href="{{ route('website.profile.settings') }}"><i class="bi bi-gear me-2"></i>Settings</a></li>
                            <li><hr class="dropdown-divider"></li>
                            <li><a class="dropdown-item text-danger" href="{{ route('user.logout') }}"><i class="bi bi-box-arrow-right me-2"></i>Logout</a></li>
                        </ul>
                    </div>
                @endguest
            </div>
...
<div class="offcanvas offcanvas-start w-offcanvas" tabindex="-1" id="wMobileMenu" aria-labelledby="wMobileMenuLabel">
    <div class="offcanvas-header border-bottom">
        <h5 class="offcanvas-title" id="wMobileMenuLabel">Menu</h5>
    ...
    <div class="offcanvas-body">
        ...
        @else
            <div class="p-3 d-grid gap-2">
                <a href="{{ route('user.login') }}" class="btn w-btn-outline">Login</a>
                <a href="{{ route('user.register') }}" class="btn w-btn-primary">Create free account</a>
            </div>
        @endauth
    </div>
</div>
